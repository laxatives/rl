...
class DispatchTest(unittest.TestCase):
    ...
    def test_sarsa(self):
        ...
        dispatcher = dispatch.Sarsa(self.alpha, self.gamma, self.idle_reward, self.open_reward)
        for _ in range(3):
    ...
    def test_dql(self):
        drivers, requests, candidates = parse.parse_dispatch(self.dispatch_observ)
        dispatcher = dispatch.Dql(self.alpha, self.gamma, self.idle_reward, self.open_reward)
        for _ in range(3):
            d = dispatcher.dispatch(drivers, requests, candidates)
            assert d
