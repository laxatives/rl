...
class StateValueGreedy(Repositioner):
    def reposition(self, data: RepositionData) -> List[Dict[str, str]]:
        ...
        for grid_id in self.dispatcher.get_grid_ids():
            value = self.dispatcher.state_value(HEX_GRID.grids[grid_id], grid_id, data.timestamp)
        ...
        for driver_id, current_grid_id in data.drivers:
            current_value = self.dispatcher.state_value(HEX_GRID.grids[current_grid_id],
                                                        current_grid_id, data.timestamp)
            ...
            for grid_candidate in candidate_grid_ids:
                ...
                proposed_value = self.dispatcher.state_value(HEX_GRID.grids[grid_candidate.grid_id],
                                                             grid_candidate.grid_id, data.timestamp + 2)
